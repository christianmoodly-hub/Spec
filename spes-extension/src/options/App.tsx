...
export function App() {
  ...
  const [structuredFields, setStructuredFields] =
    useState<ProfileStructuredFields>(() => emptyStructuredFields())
  const [lastParsedFromDump, setLastParsedFromDump] = useState<string | null>(
    null,
  )
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setRawDump(profile?.rawDump ?? '')
        setStructuredFields(
          profile?.structuredFields ?? emptyStructuredFields(),
        )
        setLastParsedFromDump(profile?.lastParsedFromDump ?? null)
      } catch (caught) {
  ...
  return (
    <main>
      ...
      <ProfileSection
        uid={user.uid}
        initialRawDump={rawDump}
        initialStructuredFields={structuredFields}
        initialLastParsed={lastParsedFromDump}
      />
      <section>
        <h2>CV & bullets</h2>
        <p className="hint">
