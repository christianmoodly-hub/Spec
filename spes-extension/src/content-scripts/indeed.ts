...
import { isIndeedJobPage, parseIndeedJob } from './parseIndeed'
import { mountSavePanel } from './savePanel'

function sendDraft(): Promise<void> {
  return new Promise((resolve, reject) => {
    chrome.runtime.sendMessage(
      { type: MSG_SAVE_DRAFT, draft: parseIndeedJob() },
...
function mount(): void {
  if (!isIndeedJobPage()) {
    document.getElementById('spes-indeed-root')?.remove()
    return
  }
  mountSavePanel({
    hostId: 'spes-indeed-root',
    title: 'Spes',
...
chrome.runtime.onMessage.addListener(
  (message: SpesRequest, _sender, sendResponse: (response: SpesResponse) => void) => {
    ...
    sendResponse({ ok: true, draft: parseIndeedJob() })
  },
)
